...
BarWidget {
  ...
  function toggleDetails(){if(!disclosureProbe.running){disclosureProbe.command=["magi-disclosure","toggle","health"];disclosureProbe.running=true}}
  ...
  Process { id:disclosureProbe; running:true; command:["magi-disclosure","status","health"]; stdout:StdioCollector{onStreamFinished:{try{root.detailed=JSON.parse(String(text)).mode==="details"}catch(e){root.detailed=false}}} }
  ...
  Motion.MotionPopupCard { anchorItem:root; bar:root.bar; owner:root; open:root.popupOpen; contentWidth:fittedContentWidth(Style.space(410)); contentHeight:fittedContentHeight(panel.implicitHeight)
    Column { id:panel; anchors.fill:parent; spacing:Style.space(8)
      ...
      Repeater { model:(root.status.items||[]).filter(function(row){return root.detailed||row.tier==="warning"||row.tier==="critical"})
      ...
      BorderSurface { width:panel.width; height:Style.space(44); activeFocusOnTab:true; color:Style.normalFillFor(root.bar.foreground,Color.accent); borderSpec:Border.controlSpec(activeFocus?"focus":"normal",root.bar.foreground,Color.accent); Accessible.role:Accessible.Button; Accessible.name:(root.detailed?"Hide":"Show")+" nominal health telemetry"; Text{anchors.centerIn:parent;text:root.detailed?"LESS":"DETAILS";color:Color.accent;font.family:root.bar.fontFamily;font.pixelSize:Style.font.caption;font.bold:true} MouseArea{anchors.fill:parent;cursorShape:Qt.PointingHandCursor;onClicked:root.toggleDetails()} Keys.onPressed:function(event){if(event.key===Qt.Key_Return||event.key===Qt.Key_Enter||event.key===Qt.Key_Space){root.toggleDetails();event.accepted=true}} }
      Text { visible:root.detailed; text:"NOMINAL POLL // 45S     ALERT POLL // 15S     UPDATE CACHE // 15M"; color:Qt.darker(root.bar.foreground,1.5); font.family:root.bar.fontFamily; font.pixelSize:Style.font.caption }
    }
  }
}
