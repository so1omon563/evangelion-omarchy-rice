...
Item{id:root;property var shell:null;property bool opened:false;property string status:"idle";property string phase:"none";property string recovery:"";property int exitCode:0;property double updatedAt:0;property string freshness:"unavailable"
...
function updateFreshness(){if(!root.updatedAt){root.freshness="unavailable";return}var age=Math.max(0,Date.now()/1000-root.updatedAt);root.freshness=age<=900?"fresh":age<=86400?"stale":"unavailable"}
function loadState(raw){try{var s=JSON.parse(String(raw));root.status=s.status;root.phase=s.phase;root.recovery=s.recovery||"";root.exitCode=s.exit_code||0;root.updatedAt=Number(s.updated_at||0);root.updateFreshness();root.opened=root.status!=="idle";if(root.status!=="active")retire.restart()}catch(e){root.status="idle";root.phase="none";root.updatedAt=0;root.freshness="unavailable";root.opened=false}}
...
Timer{interval:60000;running:true;repeat:true;onTriggered:root.updateFreshness()}
IpcHandler{target:"update-operation";function reload():string{stateView.reload();motion.refresh();return"reloading"}function state():string{return JSON.stringify({visible:root.opened,status:root.status,phase:root.phase,freshness:root.freshness,recovery:root.recovery,exitCode:root.exitCode,motionMode:motion.mode})}}
