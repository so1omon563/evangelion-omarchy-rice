...
IpcHandler{target:"update-operation";function reload():string{stateView.reload();return"reloading"}function state():string{return JSON.stringify({visible:root.opened,status:root.status,phase:root.phase,recovery:root.recovery,exitCode:root.exitCode})}}
PanelWindow{visible:root.opened;anchors{top:true;right:true;bottom:true;left:true}color:"transparent";exclusionMode:ExclusionMode.Ignore;mask:Region{}
...
Rectangle{width:430;height:root.status==="failed"?146:120;anchors.right:parent.right;anchors.rightMargin:28;anchors.top:parent.top;anchors.topMargin:58;radius:3;color:"#ed080710";border.width:2;border.color:root.status==="failed"?"#ff4055":root.status==="complete"?"#9cf23a":"#62d8ff"
