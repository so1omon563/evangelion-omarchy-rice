...
Item {
  ...
  property color selectedText: "#b8ff5a"
  readonly property int cornerRadius: Style.cornerRadius
  property string fontFamily: Style.font.menuFamily
  property int contentMargin: Style.spacing.panelPadding
  property int headerHeight: Math.max(Style.space(68), Style.font.title * 2 + Style.spacing.controlPaddingY * 2)
  ...
  function togglePinned(index) {
    ...
    var entry = root.normalizeEntry(root.history[historyIndex])
    if (!entry) return
    entry.pinned = !entry.pinned
    root.history[historyIndex] = entry
  ...
  function toggleConcealed(index) {
    if (index < 0 || index >= displayModel.count) return
    var historyIndex = displayModel.get(index).historyIndex
    var entry = root.normalizeEntry(root.history[historyIndex])
    if (!entry) return
    entry.concealed = !entry.concealed
    root.history[historyIndex] = entry
    root.history = root.history.slice()
    root.saveHistory()
    root.rebuildDisplay()
      ...
      Item {
        ...
        Keys.onPressed: function(event) {
          ...
          if (event.key === Qt.Key_Escape) {
            ...
            root.togglePinned(root.selectedIndex)
            event.accepted = true
          } else if (event.key === Qt.Key_H && (event.modifiers & Qt.ControlModifier)) {
            root.toggleConcealed(root.selectedIndex)
            event.accepted = true
        ...
        ConfirmDialog {
          ...
          z: 10
          message: "PURGE ALL CLASSIFIED CLIPBOARD DATA?"
          confirmText: "PURGE"
        ...
        Rectangle {
          ...
          Column {
            ...
            Row {
              ...
              Text {
                text: "MAGI // CLASSIFIED DATA ARCHIVE"
                color: "#9cf23a"
              ...
              Text {
                text: "[ " + root.history.length + " RECORDS ]"
                color: "#f6a52f"
                font.family: root.fontFamily
            ...
            Text {
              width: parent.width
              text: root.filterText ? "QUERY > " + root.filterText : "QUERY > Search archive…"
        ...
        Item {
          ...
          Row {
            ...
            Item {
              ...
              ListView {
                ...
                delegate: Rectangle {
                  ...
                  required property bool concealed
                  ...
                  Row {
                    ...
                    Rectangle {
                      ...
                      Text {
                        ...
                        color: row.hasCursor ? "#080710" : "#d7c8ee"
                        font.family: root.fontFamily
                        font.pixelSize: Style.font.caption
                        font.bold: true
                      }
                    ...
                    Image {
                      visible: parent.parent.previewImage.length > 0 && !parent.parent.concealed
                    ...
                    Text {
                      width: parent.width - Style.space(64) - parent.spacing -
                        (parent.parent.previewImage.length > 0 && !parent.parent.concealed ? parent.height + parent.spacing : 0)
                      height: parent.height
                      text: (parent.parent.pinned ? "󰐃  " : "") +
                        (parent.parent.concealed ? "[ PREVIEW CONCEALED ]" : parent.parent.previewText)
            ...
            Item {
              ...
              Text {
                visible: parent.activeRow && !parent.activeRow.previewImage && !parent.activeRow.concealed
                anchors.fill: parent
              ...
              Column {
                visible: parent.activeRow && parent.activeRow.concealed
                anchors.centerIn: parent
