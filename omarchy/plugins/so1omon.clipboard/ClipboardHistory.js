function normalizeEntry(value) {
  ...
  if (type === "text") {
    ...
    return { type: "text", text: text, pinned: value.pinned === true, concealed: value.concealed === true }
  }
  ...
  if (type === "image") {
    ...
    entry.pinned = value.pinned === true
    entry.concealed = value.concealed === true
    return entry
  }
...
function addEntry(history, entry, limit) {
  ...
  // Refreshing an entry must not silently discard its archive controls.
  for (var j = 0; j < values.length; j++) {
    var prior = normalizeEntry(values[j])
    if (prior && entryKey(prior) === key) {
      normalized.pinned = prior.pinned
      normalized.concealed = prior.concealed
      break
    }
  }
...
function searchableText(entry) {
  ...
  return String(entry.text || "") + " " + fileEntryText(entry) +
    (entry.pinned ? " pinned priority" : "") + (entry.concealed ? " concealed sensitive" : "")
}

function isUrlText(text) {
  return /^(https?|ftp):\/\/\S+$/i.test(String(text || "").trim())
}

function classification(entry, isFile) {
  if (entry && entry.type === "image") return "VISUAL"
  if (isFile) return "FILE"
  if (entry && isUrlText(entry.text)) return "LINK"
  return "TEXT"
}
...
function displayRows(history, query, limit) {
  ...
  for (var p = 0; p < values.length; p++) {
    var candidate = normalizeEntry(values[p])
    if (candidate && candidate.pinned) order.push(p)
  }
  for (var u = 0; u < values.length; u++) {
    var ordinary = normalizeEntry(values[u])
    if (ordinary && !ordinary.pinned) order.push(u)
  }
  ...
  for (var o = 0; o < order.length; o++) {
    ...
    rows.push({
      ...
      classification: classification(entry, isFile),
      pinned: entry.pinned === true,
      concealed: entry.concealed === true,
      index: i
    })
