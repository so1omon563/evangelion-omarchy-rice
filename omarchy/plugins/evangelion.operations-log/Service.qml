...
Item {
  ...
  function toggleDetails(){if(!disclosureProc.running){disclosureProc.command=["magi-disclosure","toggle","operations-log"];disclosureProc.running=true}}
  ...
  Process{id:disclosureProc;stdout:StdioCollector{onStreamFinished:{try{root.detailed=JSON.parse(String(text)).mode==="details"}catch(e){root.detailed=false}}}}
  ...
  PanelWindow {
    ...
    Rectangle {
      ...
      ColumnLayout{anchors.fill:parent;anchors.margins:20;spacing:10
        RowLayout{Layout.fillWidth:true;Text{text:"MAGI // OPERATIONS LOG";color:Color.accent;font.family:"JetBrainsMono Nerd Font";font.pixelSize:18;font.bold:true;font.letterSpacing:2}Item{Layout.fillWidth:true}Rectangle{Layout.preferredWidth:100;Layout.preferredHeight:44;color:Color.background;border.width:1;border.color:Color.accent;Accessible.role:Accessible.Button;Accessible.name:(root.detailed?"Hide":"Show")+" operation details";Text{anchors.centerIn:parent;text:root.detailed?"LESS // D":"DETAILS // D";color:Color.accent;font.family:"JetBrainsMono Nerd Font";font.pixelSize:8;font.bold:true}MouseArea{anchors.fill:parent;onClicked:root.toggleDetails()}}Text{text:"PRIVACY-SANITIZED";color:Color.muted;font.family:"JetBrainsMono Nerd Font";font.pixelSize:9}}
        Rectangle{Layout.fillWidth:true;Layout.preferredHeight:46;color:Color.background;border.width:1;border.color:query.activeFocus?Color.accent:Color.muted
          TextInput{id:query;anchors.fill:parent;anchors.margins:12;color:Color.foreground;selectionColor:Color.selection;font.family:"JetBrainsMono Nerd Font";font.pixelSize:13;clip:true;Accessible.role:Accessible.EditableText;Accessible.name:"Search notification and MAGI event history";Accessible.description:"Type to search, arrows navigate, Enter invokes a safe action, C clears with confirmation, E exports, Escape closes";onTextChanged:{root.selected=0;debounce.restart()}
            Keys.priority:Keys.BeforeItem;Keys.onPressed:function(event){if(event.key===Qt.Key_Escape)root.hide();else if(event.key===Qt.Key_Down)root.move(1);else if(event.key===Qt.Key_Up)root.move(-1);else if(event.key===Qt.Key_Return||event.key===Qt.Key_Enter)root.invoke();else if(event.key===Qt.Key_C&&query.text.length===0)root.requestClear();else if(event.key===Qt.Key_E&&query.text.length===0){exportProc.command=["magi-operations-log","export"];exportProc.running=true}else if(event.key===Qt.Key_D&&query.text.length===0)root.toggleDetails();else return;event.accepted=true}}
        }
        ListView{id:list;Layout.fillWidth:true;Layout.fillHeight:true;clip:true;spacing:5;model:root.entries;currentIndex:root.selected
          delegate:Rectangle{required property var modelData;required property int index;width:list.width;height:74;color:index===root.selected?Color.selection:Color.background;border.width:index===root.selected?1:0;border.color:Color.accent
            RowLayout{anchors.fill:parent;anchors.margins:10;spacing:12
              ...
              ColumnLayout{Layout.fillWidth:true;Text{Layout.fillWidth:true;text:String(modelData.summary||"");elide:Text.ElideRight;color:Color.foreground;font.family:"JetBrainsMono Nerd Font";font.pixelSize:11;font.bold:true}Text{visible:root.detailed;Layout.fillWidth:true;text:String(modelData.detail||"");elide:Text.ElideRight;color:Color.muted;font.family:"JetBrainsMono Nerd Font";font.pixelSize:9}Text{visible:root.detailed;Layout.fillWidth:true;text:String(modelData.source||"MAGI").toUpperCase();elide:Text.ElideRight;color:Color.muted;font.family:"JetBrainsMono Nerd Font";font.pixelSize:7}}
        ...
        Text{Layout.fillWidth:true;horizontalAlignment:Text.AlignHCenter;text:"↑↓ NAVIGATE · ENTER ACTION · D DETAILS · C CLEAR · E EXPORT · ESC CLOSE";color:Color.muted;font.family:"JetBrainsMono Nerd Font";font.pixelSize:8}
      }
    }
  }
}
