...
Item {
  ...
  function playSound(cue) {
    if (soundProcess.running) return
    soundProcess.command = ["magi-sound", cue]
  ...
  function approveUnlock() {
    ...
    logEvent("authorization-granted")
  }
  ...
  WlSessionLock {
    ...
    WlSessionLockSurface {
      ...
      LockView {
        ...
        authorizationGranted: root.authorizationGranted
  ...
  PanelWindow {
    ...
    LockView {
      ...
      backgroundVersion: root.backgroundVersion
      fingerprintConfigured: root.fingerprintConfigured
      authenticatingPassword: root.previewMode === 1
      authorizationGranted: root.previewMode === 3
      failureMessage: root.previewMode === 2 ? "Authentication failed (1)" : ""
      failedAttempts: root.previewMode === 2 ? 1 : 0
      inputEnabled: false
  ...
  PamContext {
    ...
    onCompleted: function(result) {
      ...
      if (!root.lockRequested) return
      if (result === PamResult.Success) root.approveUnlock()
  ...
  IpcHandler {
    ...
    function previewChecking(): string {
      root.previewMode = 1
      root.previewVisible = true
      return "ok"
    }

    function previewFailure(): string {
      root.previewMode = 2
      root.previewVisible = true
      return "ok"
    }

    function previewGranted(): string {
      root.previewMode = 3
      root.previewVisible = true
      return "ok"
    }
