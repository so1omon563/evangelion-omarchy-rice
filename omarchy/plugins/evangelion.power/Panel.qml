...
Panel {
  ...
  property int previewPercent: -1
  property int previewMode: 0 // 0=live, 1=discharging, 2=charging
  ...
  function displayBatteryIcon() {
    if (root.previewMode === 2) return "󰂉"
    if (root.previewMode === 1 && root.previewPercent <= 7) return "󰂃"
  ...
  readonly property int batteryPercent: root.previewPercent >= 0 ? root.previewPercent : Math.round(root.batteryFraction * 100)
  readonly property bool displayDischarging: root.previewMode === 1 || (root.previewMode === 0 && root.discharging)
  readonly property bool displayCharging: root.previewMode === 2 || (root.previewMode === 0 && root.charging)
  readonly property bool reservePower: root.displayDischarging && root.batteryPercent <= 15
  readonly property bool internalPower: root.displayDischarging && root.batteryPercent <= 7
  readonly property color batteryStateColor: root.internalPower
    ? "#FF4055"
    : root.reservePower
      ? "#F6B73C"
  ...
  readonly property string heroStatusText: {
    if (internalPower) return "INTERNAL POWER"
    if (reservePower) return "Reserve power"
    if (displayCharging) return "External power // charging"
  ...
  IpcHandler {
    ...
    function previewReserve() { root.previewPercent = 15; root.previewMode = 1 }
    function previewCritical() { root.previewPercent = 7; root.previewMode = 1 }
    function previewCharging() { root.previewPercent = 63; root.previewMode = 2 }
    function clearPreview() { root.previewPercent = -1; root.previewMode = 0 }
  }
  ...
  BarIconButton {
    ...
    anchors.fill: parent
    bar: root.bar
    text: root.internalPower && !vertical
      ? "INTERNAL POWER " + root.batteryPercent + "%"
      : root.reservePower && !vertical
        ? root.batteryPercent + "% " + root.displayBatteryIcon()
        : root.showPercentage && !vertical
          ? root.batteryPercent + "% " + root.displayBatteryIcon()
          : root.displayBatteryIcon()
    slotSize: Style.bar.iconSlot * (root.internalPower && !vertical ? 6 : ((root.reservePower || root.showPercentage) && !vertical ? 2 : 1))
    foreground: root.batteryStateColor
    tooltipText: root.internalPower
      ? "INTERNAL POWER // " + root.batteryPercent + "% remaining"
      : root.reservePower
        ? "Reserve power // " + root.batteryPercent + "% remaining"
        : root.displayCharging
          ? "External power // charging at " + root.batteryPercent + "%"
          : root.batteryPercent + "% remaining"
      ...
      Column {
        ...
        Item {
          ...
          Column {
            ...
            Text {
              ...
              color: Qt.darker(root.batteryStateColor, 1.25)
              font.family: root.bar.fontFamily
        ...
        Item {
          ...
          Rectangle {
            ...
            anchors.fill: parent
            radius: height / 2
            color: Qt.rgba(root.batteryStateColor.r, root.batteryStateColor.g, root.batteryStateColor.b, 0.12)
          }
